...
(async () => {
	...
	Object.entries(DATA[MAIN]).forEach(([_, main]) => {
		...
		if(fs.existsSync(output) && new Date(feature.LastModifiedTime).getTime() > fs.statSync(output).mtime.getTime()){
	...
	const items = fs.readdirSync("features")
					...
					.filter(slug => fs.existsSync(`public/images/${slug}.png`) && fs.statSync(`features/${slug}.json`).mtime.getTime() > fs.statSync(`public/images/${slug}.png`).mtime.getTime())
